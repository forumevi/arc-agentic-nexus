...
<<<<<<< HEAD
      ...
      if (!accounts || accounts.length === 0) {
        throw new Error('No accounts returned from MetaMask or wallet request was cancelled.');
      }
      ...
      try {
        ...
        if (network && Number(network.chainId) !== ARC_TESTNET_CONFIG.chainId) {
          if (typeof ethProvider.request === 'function') {
            try {
        ...
        console.warn('Network setup warning:', chainErr);
      }
...
<<<<<<< HEAD
      ...
      } else if (msg.includes('Cannot redefine property') || msg.includes('isZerion')) {
        msg = 'Multiple wallet extensions detected (e.g. Zerion/MetaMask conflict). You can connect using a Simulated Testnet Account.';
      } else {
...
<<<<<<< HEAD
            <div className="p-4 bg-black border border-rose-500 text-rose-300 text-xs font-mono space-y-2">
              ...
              <button
                ...
                className="mt-1 px-3 py-1.5 bg-rose-500/20 hover:bg-rose-500 hover:text-white text-rose-300 border border-rose-500/50 text-[11px] font-bold uppercase transition flex items-center gap-1.5"
              >
